...
const makeElectronAppLayer = (calls: ElectronAppCalls) =>
  Layer.succeed(ElectronApp.ElectronApp, {
    ...
    name: Effect.succeed("Naso Code Lab"),
      ...
      DesktopAppIdentity.layer.pipe(
        Layer.provideMerge(
          FileSystem.layerNoop({
            exists: (path) =>
              Effect.succeed(
                input.legacyPathExists === true && path.includes("Naso Code Lab (Alpha)"),
              ),
            readFileString: () =>
    ...
    withIdentity(
      Effect.gen(function* () {
        ...
        assert.equal(
          ...
          "/Users/alice/Library/Application Support/Naso Code Lab (Alpha)",
        );
      }),
  ...
  it.effect("configures app identity from the environment commit override", () => {
    ...
    return withIdentity(
      Effect.gen(function* () {
        ...
        assert.deepEqual(calls.setName, ["Naso Code Lab (Alpha)"]);
        assert.equal(calls.setAboutPanelOptions[0]?.applicationName, "Naso Code Lab (Alpha)");
